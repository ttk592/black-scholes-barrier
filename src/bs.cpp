...
// we may want to set assembly markers in order to examine the
// efficiency of the assembly output
#ifndef ASM_COMMENT
#define ASM_COMMENT(X)  asm volatile ("#" X)
#endif


// the code has been written with the following objectives in mind:
// - avoid object-oriented patterns so that the code could quickly
//   be translated to C, if required
// - minimise code replication
// - make use of the building blocks in pricing options, e.g.
//    - call option = asset-or-nothing - K * cash-or-nothing option
//    - barrier options can be priced via the respective vanilla options
//      with truncated payoff
// - easy to extend for barrier options on any other payoff g(S_T)
//
//
...
//
// other inefficiency: if we wanted to know all greeks at once, then
//   we calculate many of the same terms over and over again. I was hoping
//   using strong inlining the compiler would remove this but unfortunately
//   it doesn't manage
//
//
...
inline double pnorm(double x) {
   // erf/erfc is part of C99 and C++11, supported by gcc + glibc
   // return 0.5*(erf(sqrt(0.5)*x)+1.0);
...
double bincash(double S, double vol, double rd, double rf,
   ...
   if(tau<=0.0) {
      ...
      switch(greeks) {
      case types::Value:
         ASM_COMMENT("begin binary value"); // just to see how pm*x is handled
         val = pnorm(pm*d2);
         ASM_COMMENT("end binary val");
         break;
      case types::Delta:
...
double barrier(double S, double vol, double rd, double rf,
   ...
   if(rebate!=0.0) {
      // a rebate can be seen as a touch option (domestic)
      // but with the opposite barrier type (knock-in/out) as the original
      // barrier option
